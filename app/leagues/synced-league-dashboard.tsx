...
export function SyncedLeagueDashboard({data}:{data:LeagueSnapshot}){
  ...
  return <>
    ...
    <section className="section league-hub" id="league-dashboard">
      ...
      {tab==="lanes"&&laneTable&&<div className="lane-assignments"><div className="recap-heading"><div><p className="eyebrow red">Week {Number(week)+1}</p><h3>Lane assignments</h3></div><p>Upcoming Wednesday matchups, listed in lane order.</p></div><div className="lane-assignment-grid">{laneTable.rows.filter(row=>!q||row.join(" ").toLowerCase().includes(q)).map((row,index)=>{const lane=cell(laneTable,row,"Lane")||row[0],team=cell(laneTable,row,"Team#")||row[1],name=cell(laneTable,row,"Team")||row[2]||`Team ${team}`;return <article key={`${lane}-${team}-${index}`}><b>{lane}</b><span><small>Team #{team}</small><strong>{name}</strong></span></article>;})}</div></div>}
    </section>
  </>;
}
